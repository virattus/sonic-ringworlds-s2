...
static inline int16_t int16_interpolate(int16_t s0, int16_t s1, uint16_t numerator, uint16_t denominator)
{
	return fix16_to_int(fix16_lerp16(fix16_from_int(s0), fix16_from_int(s1), (numerator << 16) / denominator));
}


static inline int32_t int32_interpolate(int32_t i0, int32_t i1, uint16_t numerator, uint16_t denominator)
{
